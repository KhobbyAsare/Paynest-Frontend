...
import { Search, RefreshCcw, Wallet, Building2, UserCog } from 'lucide-react';
...
export default function PayrollEmployeesPage() {
    ...
    const fetchUsers = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const data = await getOrganizationUsers(isSuperadmin ? selectedOrgId ?? undefined : undefined);
            setUsers(data);
        } catch (err) {
    ...
    }, [isSuperadmin, selectedOrgId, orgReady]);
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            {!orgReady ? (
                <EmptyState icon={Building2} title="Select an organization" description="Pick an organization above to view its employees." />
            ) : (
                <>
