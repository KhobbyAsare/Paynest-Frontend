...
import { Modal, Form, Input, Button, Card, message, Select, DatePicker, Tag, Tooltip, Badge, Descriptions } from 'antd';
import { Plus, FileText, Calendar, Search, Clock, CheckCircle, XCircle, Eye } from 'lucide-react';
import { ReportFileFormat, ReportRequest, ReportResponse, ReportStatus, ReportType } from '@/interfaces/report';
import { createReport, getMyResports, getReportByID } from '@/(api-handlers)/reportHandler';
...
export default function ManagerReportView() {
    ...
    const handleCreateReport = async (values: ReportFormValues) => {
        ...
        try {
            ...
            message.success('Report generation initiated successfully');
    ...
    const getStatusBadge = (status: ReportStatus) => {
        const statusConfig: Record<string, { color: string; icon: React.ReactElement; text: string }> = {
            pending: { color: 'warning', icon: <Clock className="size-3" />, text: 'Pending' },
            approved: { color: 'success', icon: <CheckCircle className="size-3" />, text: 'Approved' },
        ...
        return (
            <Badge
                status={config.color as 'success' | 'processing' | 'error' | 'warning'}
                text={
